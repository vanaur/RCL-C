...
const String rcl_get_current_version_s()
{
    return show_version(CURRENT_VERSION);
}

const Version rcl_get_current_version_v()
{
    return CURRENT_VERSION;
}

_export struct IResult rcl_simple_interpret_ir_s(const String p_s, const bool show_res)
{
    ...
    if (bresult.resulting.used == 1)
    {
        ...
        else if (bresult.resulting.array[0].kind == RCL_Value_Float)
        {
            res.main_returned = mpf_get_d(bresult.resulting.array[0].u.float_);
            if (!bresult.state.hasError)
                res.program_res = SUCCESS;
            else
                res.program_res = FAILURE;
...
struct IResult rcl_evali_expr(const rcl_expr_t expr)
{
    struct IResult res;
